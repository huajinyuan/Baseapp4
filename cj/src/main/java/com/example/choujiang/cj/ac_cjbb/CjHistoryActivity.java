...
public class CjHistoryActivity extends BaseActivity<CjHistoryPresenter> {
    Context context;
    ACache aCache;
    public String token;
    ...
    void getData(){
        HttpMethods.start(HttpMethods.getInstance().demoService.getCjHistory(token, page, 10), new Subscriber<Response<ArrayList<CjHistory>>>() {
            @Override
            public void onStart() {
                super.onStart();
                canGet = false;
