...
import com.example.choujiang.cj.ac_memberget.adapter.MemberGetListAdapter;
...
public class MemberGetListActivity extends BaseActivity<MemberGetListPresenter> {
    ...
    protected int getLayoutId() {
        return R.layout.activity_member_get_list_cj;
    }
    ...
    void getData(){
        HttpMethods.start(HttpMethods.getInstance().demoService.getMembergetList_cj(token, page, 10), new Subscriber<Response<ArrayList<CjHistory>>>() {
            @Override
            public void onStart() {
                super.onStart();
                canGet = false;
