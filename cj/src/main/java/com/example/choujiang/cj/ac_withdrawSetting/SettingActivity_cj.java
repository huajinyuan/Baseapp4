...
public class SettingActivity_cj extends BaseActivity<SettingPresenter_cj> {
    ...
    void getData(){
        HttpMethods.start(HttpMethods.getInstance().demoService.getSetting(token, "3"), new Subscriber<Response<WithdrawSetting>>() {
            @Override
            public void onStart() {
                super.onStart();
            }
    ...
    void saveSetting(int type, int value) {
        HttpMethods.start(HttpMethods.getInstance().demoService.saveSetting(token, "3", type, value), new Subscriber<Response>() {
            @Override
            public void onStart() {
                super.onStart();
            }
