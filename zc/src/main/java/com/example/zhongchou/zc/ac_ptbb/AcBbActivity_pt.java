...
import com.example.zhongchou.zc.ac_ptbb.adapter.AcBbAdapter_pt;
...
public class AcBbActivity_pt extends BaseActivity<AcBbPresenter_pt> {
    ...
    void setRv(ArrayList<Activity_pt> pinDans) {
        if (adapter == null) {
            ...
            adapter = new AcBbAdapter_pt(context, pinDan_pts);
    ...
    void getData(){
        HttpMethods.start(HttpMethods.getInstance().demoService.getAcReport_pt(token, page, 10, requestStatus), new Subscriber<Response<ArrayList<Activity_pt>>>() {
            @Override
            public void onStart() {
                super.onStart();
                canGet = false;
